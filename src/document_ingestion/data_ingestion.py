from __future__ import annotations #to handle the circular imports or future references
import os
import sys
import json
import uuid #universal identification number 
#to generate unique id's, data versioning - at every run , how 
# to keep the specific version of data in uuid folder
import hashlib
import shutil
from pathlib import Path
from typing import Iterable, List, Optional, Dict, Any
import fitz  # PyMuPDF - like a wrapper on top of PyMuPDF library
...
class FaissManager:
    ...
    def _fingerprint(text: str, md: Dict[str, Any]) -> str:
        #no duplicate entry available in the faiss database
...
#logic for creating the session directory    for every execution     
        self.data_dir = data_dir or os.getenv("DATA_STORAGE_PATH", os.path.join(os.getcwd(), "data", "document_analysis"))
#session_id - for every execution, we are going to save the data and that data is available 
# under session_id which is a universal unique id     
# we never delete the data in production, we maintain archiving ie. Data versioning wrt every execution 
#to avoid data redundancy and maintain archiving in the session folders
#data archival strategy/data versioning
        self.session_id = session_id or generate_session_id("session")
        self.session_path = os.path.join(self.data_dir, self.session_id)
        os.makedirs(self.session_path, exist_ok=True)
        log.info("DocHandler initialized", session_id=self.session_id, session_path=self.session_path)

#logic for saving the file in the session folder
    ...
    def read_pdf(self, pdf_path: str) -> str:
        try:
            text_chunks = []
            #we can also use langchain for loading the pdf files
...
class DocumentComparator:
    ...
    def save_uploaded_files(self, reference_file, actual_file):
        # actual_file=v1 and reference_file=v2
        try:
...
if __name__ == "__main__":
    from pathlib import Path #makes system compatible path
    from io import BytesIO #this is for buffer, generally, for faster processing, we process in memory, then we keep in the disk
    handler = DocHandler()

    pdf_path=r"C:\\Users\\singupil\\OneDrive - Adobe\\Desktop\\Learning\\AI\\Learning Projects\\LLMOps\\document_portal\\data\\document_analysis\\attention.pdf"
    ...
    class DummyFile:
        ...
        def getbuffer(self):
            return open(self._file_path, "rb").read() #open the file in the memory itself
        
    dummy_pdf = DummyFile(pdf_path)   

    handler = DocHandler(session_id="test_session") #here the folder will be created with name test_session
