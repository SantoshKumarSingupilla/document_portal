#create a application programming interface for our entire backend code
#we can serve our entire backend code throught the browser itself
...
#created a object for Fast API class
app = FastAPI(title="Document Portal API", version="0.1") #the parameter values should be string only
...
#below are the different routes created
#home route
@app.get("/", response_class=HTMLResponse)
async def serve_ui(request: Request): #it will not work in a queue, so that it can run parallely
...
async def analyze_document(file: UploadFile = File(...)) -> Any:
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Analysis failed: {e}") #500 means internal server error
